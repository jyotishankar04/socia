...
import type TokenService from "../user/token.service";
import type { OtpService } from "../user/otp.service";
...
export class AuthController {
    ...
    private async sendVerificationOtp(email: string, userId: string, next: NextFunction) {
        try {
            await this.otpService.sendOtp(email, userId)
            return true
        } catch (err) {
            next(createHttpError(503, "Could not send the verification email. Make sure the mail service is running and try again.", { cause: err }))
            return false
        }
    }

    public async register(req: Request, res: Response, next: NextFunction) {
        const { data: body, success: validationSuccess, error } = await registerSchema.safeParseAsync(req.body)
        if (!validationSuccess) {
            next(createHttpError(400, error.issues[0]?.message ?? "Validation failed"))
            return;
        }
        ...
        if (user) {
            if (user.isVerified) {
                ...
                if (!await this.sendVerificationOtp(user.email, user.id, next)) return;
                return res.status(201).json({
                    success: true,
                    data: {
                        redirectEndpoint: `/auth/otp-verification?userId=${user.id}&purpose=register`,
                    },
        ...
        if (!await this.sendVerificationOtp(data.email, data.id, next)) return;
    ...
    public async verifyOtp(req: Request, res: Response, next: NextFunction) {
        const { userId, otp: code } = req.body;
        ...
        let otpResult: { success: boolean, message: string };
        try {
            otpResult = await this.otpService.verifyOtp(userId, code);
        } catch (err) {
        ...
        if (!otpResult.success) {
            next(createHttpError(400, otpResult.message));
            return;
        }
